  ...
  class PocketsphinxServer::Server < Sinatra::Base
    ...
    configure do
      ...
      config['handlers'].each do |handler_config|
        class_name = handler_config['name']
        # TODO requireName = handler_config['require']
        # TODO require "#{requireName}"
        puts "Instantiating handler #{class_name}..."
        handler = PocketsphinxServer::Handlers.const_get(class_name).new(self, handler_config)  
        handlers << handler    
      end
    ...
    # Parses Content-type and resolves it to GStreamer Caps string
